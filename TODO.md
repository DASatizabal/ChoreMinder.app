...
- [x] Audit all 141+ files importing from `@/libs/` to ensure consistency (FIXED - all imports now use @/libs/)
- [x] Consider consolidating `/lib/` and `/libs/` into a single directory (FIXED - /lib/ deleted, all files moved to /libs/)
- [x] Update all imports to use consistent path aliases (FIXED - 17 files updated)

### Duplicate Configurations
- [x] Consolidate auth configuration (`/lib/auth.ts` and `/libs/next-auth.ts`) (FIXED - libs/next-auth.ts is canonical, lib/ deleted)
- [x] Remove duplicate database connection patterns (`/lib/dbConnect.ts` vs `/libs/mongoose.ts`) (FIXED - libs/mongoose.ts is canonical, lib/ deleted)
- [x] Standardize MongoDB client usage across the codebase (FIXED - libs/mongoose.ts for Mongoose, libs/mongo.ts for NextAuth adapter)

### Unused Dependencies
- [x] Remove Prisma from `package.json` (not used - Mongoose is the ORM) (FIXED)
- [x] Delete `/prisma/` directory and schema file (FIXED)
- [x] Audit `package.json` for other unused dependencies (FIXED - removed @mdx-js/loader, @mdx-js/react, @next/mdx, nodemailer, react-syntax-highlighter, @types/mdx, @types/react-syntax-highlighter)

### Incomplete Features
- [x] Replace placeholder landing page (`/app/page.tsx`) with actual content (FIXED)
- [ ] Complete TODO items in `libs/stripe.ts`
- [ ] Finish reminder scheduler logic in `libs/reminder-scheduler.ts`
- [x] Address mock data TODO in `app/api/chores/route.ts` (FIXED - replaced with real database queries)
