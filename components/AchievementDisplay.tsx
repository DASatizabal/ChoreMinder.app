...
const AchievementDisplay = ({
  ...
  const fetchUserData = async () => {
    try {
      ...
      const recentlyEarned = allAchievements.filter(
        (a) =>
          a.earned &&
          a.earnedDate &&
  ...
  const calculateUserStats = (chores: any[]): UserStats => {
    ...
    const recentPoints = recentChores.reduce(
      (sum, c) => sum + (c.points || 0),
      0,
    );
  ...
  const generateAchievements = (stats: UserStats): Achievement[] => {
    return [
      // Milestone Achievements
      {
        ...
        earnedDate:
          stats.completedChores >= 1 ? new Date().toISOString() : undefined,
        points: 10,
      },
      {
        id: "early_bird",
    ...
    <div className={`${showAnimation ? "animate-pulse" : ""}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="stat bg-gradient-to-br from-purple-100 to-pink-100 rounded-xl shadow-lg border-2 border-purple-200">
          ...
          <div className="stat-value text-purple-800">
        ...
        <div className="alert alert-success mb-6 animate-bounce">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p>
                You earned {newAchievements.length} new achievement
                {newAchievements.length > 1 ? "s" : ""} recently! You're
                amazing!
              </p>
            </div>
          </div>
            ...
            >
              ...
              {category.id !== "all" && (
                <div className="badge badge-neutral ml-2">
                  {
                    achievements.filter(
                      (a) => a.category === category.id && a.earned,
                    ).length
                  }
                </div>
              )}
        ...
        {filteredAchievements.map((achievement) => (
          ...
          >
            <div className="card-body p-6 text-center">
              <div
                className={`text-6xl mb-4 ${achievement.earned ? "" : "opacity-50 grayscale"}`}
              >
                {achievement.emoji}
              </div>
