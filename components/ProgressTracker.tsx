...
const ProgressTracker = ({ familyId, familyMembers }: ProgressTrackerProps) => {
  ...
  const [familyTrend, setFamilyTrend] = useState<Array<{
    date: string;
    completed: number;
    assigned: number;
    completionRate: number;
  ...
  const fetchProgressData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/chores?familyId=${familyId}&analytics=true&timeframe=${timeframe}`);
  ...
  const processProgressData = (chores: any[]) => {
    ...
    const recentChores = chores.filter(chore => 
      new Date(chore.createdAt) >= cutoffDate
    );

    // Calculate family stats
    const familyStats: ChoreStats = {
      totalChores: recentChores.length,
      completedChores: recentChores.filter(c => ["completed", "verified"].includes(c.status)).length,
      pendingChores: recentChores.filter(c => c.status === "pending").length,
      overdueChores: recentChores.filter(c => 
        c.dueDate && new Date(c.dueDate) < now && !["completed", "verified"].includes(c.status)
      ).length,
      ...
      completionRate: recentChores.length > 0 ? 
        (recentChores.filter(c => ["completed", "verified"].includes(c.status)).length / recentChores.length) * 100 : 0,
    };

    // Calculate member progress
    const memberProgress: MemberProgress[] = children.map(member => {
      const memberChores = recentChores.filter(c => c.assignedTo?._id === member.user._id);
      const completedChores = memberChores.filter(c => ["completed", "verified"].includes(c.status));
      ...
      return {
        ...
        stats: {
          ...
          pendingChores: memberChores.filter(c => c.status === "pending").length,
          overdueChores: memberChores.filter(c => 
            c.dueDate && new Date(c.dueDate) < now && !["completed", "verified"].includes(c.status)
          ).length,
          totalPoints: memberChores.reduce((sum, c) => sum + (c.points || 0), 0),
          earnedPoints: completedChores.reduce((sum, c) => sum + (c.points || 0), 0),
          averageCompletionTime: calculateAverageCompletionTime(memberChores),
          completionRate: memberChores.length > 0 ? (completedChores.length / memberChores.length) * 100 : 0,
        },
  ...
  const calculateAverageCompletionTime = (chores: any[]) => {
    const completedWithTime = chores.filter(c => 
      c.completedAt && c.createdAt && ["completed", "verified"].includes(c.status)
    );
  ...
  const calculateWeeklyProgress = (memberId: string, allChores: any[]) => {
    ...
    return days.map((day, index) => {
      ...
      return {
        day,
        completed: dayChores.filter(c => ["completed", "verified"].includes(c.status)).length,
  ...
  const calculateFamilyTrend = (chores: any[], days: number) => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const completed = dayChores.filter(c => ["completed", "verified"].includes(c.status)).length;
  ...
  const getTrendIcon = (trend: "up" | "down" | "stable") => {
    switch (trend) {
      case "up":
        return <svg className="w-4 h-4 text-success" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M3.293 9.707a1 1 0 010-1.414l6-6a1 1 0 011.414 0l6 6a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L4.707 9.707a1 1 0 01-1.414 0z" clipRule="evenodd" />
        </svg>;
      case "down":
        return <svg className="w-4 h-4 text-error" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M16.707 10.293a1 1 0 010 1.414l-6 6a1 1 0 01-1.414 0l-6-6a1 1 0 111.414-1.414L9 14.586V3a1 1 0 012 0v11.586l4.293-4.293a1 1 0 011.414 0z" clipRule="evenodd" />
        </svg>;
      default:
        return <svg className="w-4 h-4 text-info" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-base-content/70">Monitor family chore completion and performance</p>
        </div>
      ...
      {selectedMember === "all" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-value text-primary">{familyStats.completionRate.toFixed(1)}%</div>
            <div className="stat-desc">{familyStats.completedChores} of {familyStats.totalChores} chores</div>
          </div>
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-value text-secondary">{familyStats.earnedPoints}</div>
            <div className="stat-desc">of {familyStats.totalPoints} possible</div>
          </div>
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-value text-warning">{familyStats.averageCompletionTime.toFixed(1)}h</div>
          ...
          <div className="stat bg-base-200 rounded-lg shadow">
            ...
            <div className="stat-value text-error">{familyStats.overdueChores}</div>
      ...
      {selectedMember === "all" ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {memberProgress.map((mp) => (
            <div key={mp.member.user._id} className="card bg-base-100 shadow-lg">
              <div className="card-body">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="avatar">
                      <div className="w-12 h-12 rounded-full">
                        {mp.member.user.image ? (
                          <img src={mp.member.user.image} alt={mp.member.user.name} />
                        ) : (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-base-content/70 capitalize">{mp.member.role}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {getTrendIcon(mp.trend)}
                    <span className="text-sm font-medium">{mp.stats.completionRate.toFixed(1)}%</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <div className="stat-value text-success text-lg">{mp.stats.completedChores}</div>
                    <div className="stat-desc">Completed</div>
                  </div>
                  <div>
                    <div className="stat-value text-warning text-lg">{mp.stats.pendingChores}</div>
                ...
                {mp.stats.earnedPoints > 0 && (
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{mp.stats.earnedPoints} / {mp.stats.totalPoints}</span>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="avatar">
                  <div className="w-16 h-16 rounded-full">
                    {selectedMemberData.member.user.image ? (
                      <img src={selectedMemberData.member.user.image} alt={selectedMemberData.member.user.name} />
                    ) : (
                      <div className="bg-primary text-primary-content flex items-center justify-center text-xl">
                        {selectedMemberData.member.user.name.charAt(0).toUpperCase()}
                      </div>
                    )}
                  </div>
                </div>
                <div>
                  <h3 className="text-xl font-bold">{selectedMemberData.member.user.name}</h3>
                  <p className="text-base-content/70 capitalize">{selectedMemberData.member.role}</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="stat">
                  ...
                  <div className="stat-value text-info text-2xl">
                    {selectedMemberData.stats.averageCompletionTime.toFixed(1)}h
                  </div>
                </div>
                <div className="stat">
            ...
            <div className="card-body">
              ...
              <div className="space-y-3">
                {selectedMemberData.recentChores.map((chore) => (
                  <div key={chore._id} className="flex items-center justify-between p-2 bg-base-200 rounded">
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className={`badge badge-sm ${
                        chore.status === "completed" || chore.status === "verified" ? "badge-success" :
                        chore.status === "pending" ? "badge-warning" : "badge-error"
                      }`}>
                        {chore.status}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-1 flex-1 justify-end">
                      {day.assigned > 0 && (
                        <div 
                          ...
                          style={{ height: `${(day.assigned / Math.max(...familyTrend.map(d => d.assigned), 1)) * 100}px` }}
                      ...
                      {day.completed > 0 && (
                        <div 
                          ...
                          style={{ height: `${(day.completed / Math.max(...familyTrend.map(d => d.assigned), 1)) * 100}px` }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {familyStats.completionRate > 80 && (
              <div className="alert alert-success">
                ...
                <span className="text-sm">Great job! Your family has an excellent completion rate.</span>
              </div>
            )}
            ...
            {familyStats.overdueChores > 0 && (
              <div className="alert alert-warning">
                ...
                <span className="text-sm">
                  {familyStats.overdueChores} chore{familyStats.overdueChores > 1 ? "s are" : " is"} overdue. 
            ...
            {familyStats.completionRate < 50 && (
              <div className="alert alert-info">
                ...
                <span className="text-sm">
                  Consider breaking down complex chores or adding more incentives to improve completion rates.
                </span>
              </div>
            )}
            ...
            {memberProgress.some(mp => mp.trend === "down") && (
              <div className="alert alert-warning">
                ...
                <span className="text-sm">
                  Some family members show declining performance. Consider one-on-one check-ins.
                </span>
              </div>
            )}
          </div>
