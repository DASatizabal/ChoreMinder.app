...
export async function GET(req: NextRequest) {
  try {
    ...
    const challenges = await gamificationService.getUserChallenges(session.user.id);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === "weekly") {
      const challenge = await gamificationService.createWeeklyPersonalChallenge(session.user.id);
      return NextResponse.json({
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const challengeIndex = user.gamification?.challenges?.findIndex(
    ...
    if (challengeIndex >= 0) {
      ...
      await User.findByIdAndUpdate(
        ...
        {
          $set: {
            [`gamification.challenges.${challengeIndex}.currentProgress`]: progress,
            [`gamification.challenges.${challengeIndex}.isCompleted`]: isCompleted,
            ...(isCompleted && {
