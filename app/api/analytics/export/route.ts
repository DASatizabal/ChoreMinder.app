...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!["parent", "admin"].includes(user.role)) {
      return NextResponse.json(
        { error: "Only parents can export family reports" },
        { status: 403 },
      );
    }
    ...
    const reportData = await analyticsService.exportFamilyReport(
    ...
    return NextResponse.json(
      { error: "Invalid format. Use 'json', 'csv', or 'summary'" },
      { status: 400 },
    );
  } catch (error: any) {
...
function generateCSVReport(reportData: any): string {
  ...
  lines.push(
    `Average Completion Rate,${reportData.summary.overview.averageCompletionRate}%`,
  );
  lines.push(`Active Children,${reportData.summary.overview.activeChildren}`);
  lines.push(
    `This Week Improvement,${reportData.summary.overview.thisWeekImprovement}%`,
  );
  lines.push("");
  ...
  lines.push(
    "Name,Completion Rate,Total Points,Level,Streak,This Week Chores,Trend",
  );

  reportData.memberData.forEach((member: any) => {
    lines.push(
      `${member.name},${member.completionRate}%,${member.totalPoints},${member.level},${member.streak},${member.thisWeekChores},${member.trend}`,
    );
  });
...
function generateSummaryReport(reportData: any): string {
  ...
  lines.push(
    `📋 Total Chores: ${reportData.summary.overview.totalFamilyChores}`,
  );
  lines.push(
    `⭐ Total Points: ${reportData.summary.overview.totalFamilyPoints}`,
  );
  lines.push(
    `✅ Average Completion: ${reportData.summary.overview.averageCompletionRate}%`,
  );
  lines.push(
    `👶 Active Children: ${reportData.summary.overview.activeChildren}`,
  );
  lines.push(
    `📊 Weekly Improvement: ${reportData.summary.overview.thisWeekImprovement > 0 ? "+" : ""}${reportData.summary.overview.thisWeekImprovement}%`,
  );
  lines.push("");
  ...
  lines.push("-".repeat(30));
  lines.push(
    `🥇 Top Performer: ${reportData.summary.topInsights.topPerformer}`,
  );
  lines.push(
    `📈 Most Improved: ${reportData.summary.topInsights.mostImproved}`,
  );
  lines.push(
    `🔥 Consistency Champion: ${reportData.summary.topInsights.consistencyChampion}`,
  );
  lines.push("");
  ...
  lines.push("-".repeat(30));
  reportData.summary.topInsights.suggestions.forEach(
