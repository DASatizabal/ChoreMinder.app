...
const ERROR_MESSAGES: Record<ErrorCode, string> = {
  ...
  [ERROR_CODES.INVALID_FILE_TYPE]: "Please select a valid image file (JPG, PNG, etc.).",
  [ERROR_CODES.FILE_TOO_LARGE]: "File is too large. Please choose a smaller image.",
  
  [ERROR_CODES.CHORE_NOT_FOUND]: "Chore not found. It may have been deleted or moved.",
  [ERROR_CODES.CHORE_ALREADY_COMPLETED]: "This chore has already been completed.",
  [ERROR_CODES.INSUFFICIENT_PERMISSIONS]: "You don't have permission to perform this action.",
  [ERROR_CODES.FAMILY_NOT_FOUND]: "Family not found. Please check your family membership.",
  [ERROR_CODES.MEMBER_NOT_FOUND]: "Family member not found.",
  
  [ERROR_CODES.PHOTO_UPLOAD_FAILED]: "Failed to upload photo. Please try again.",
  [ERROR_CODES.AI_ANALYSIS_FAILED]: "AI analysis is temporarily unavailable, but you can still submit.",
  [ERROR_CODES.PHOTO_QUALITY_TOO_LOW]: "Photo quality is too low. Please take a clearer photo.",
  
  [ERROR_CODES.SETUP_INCOMPLETE]: "Setup is incomplete. Please finish the onboarding process.",
  [ERROR_CODES.INVITATION_FAILED]: "Failed to send invitation. Please try again.",
  
  [ERROR_CODES.UNKNOWN_ERROR]: "Something went wrong. Please try again or contact support."
};
  ...
  if (showToast) {
    ...
    if (suggestions && suggestions.length > 0) {
      setTimeout(() => {
        toast(
          <div>
            <div className="font-semibold mb-2">💡 Try this:</div>
...
const getErrorCodeFromStatus = (status: number): ErrorCode => {
  switch (status) {
    ...
    case 408: return ERROR_CODES.API_TIMEOUT;
    case 413: return ERROR_CODES.FILE_TOO_LARGE;
    case 422: return ERROR_CODES.INVALID_INPUT;
    case 500:
    case 502:
    case 503:
...
const logErrorToService = (error: Error, code: ErrorCode, context?: ErrorContext) => {
  try {
    const errorReport = {
...
export const validateChoreData = (choreData: any) => {
  ...
  if (typeof choreData.points !== 'number' || choreData.points < 1 || choreData.points > 100) {
    throw createChoreError(
