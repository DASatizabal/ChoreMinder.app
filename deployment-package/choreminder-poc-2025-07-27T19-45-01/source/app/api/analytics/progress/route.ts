  ...
  try {
    ...
    const canView =
      ...
      (requestingUser.role === "parent" &&
        requestingUser.familyId?.toString() ===
          targetUser.familyId?.toString()) || // Parent viewing child
    ...
    const progressMetrics = await analyticsService.getUserProgress(
      userId,
      timeRange,
    );
    ...
    // Get trend insights
    const trendInsights = await analyticsService.getTrendInsights(
      userId,
      timeRange,
    );
