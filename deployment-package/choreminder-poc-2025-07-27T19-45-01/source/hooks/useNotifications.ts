...
interface NotificationTrigger {
  choreId: string;
  type: "assigned" | "accepted" | "declined" | "in_progress" | "completed" | "photo_submitted" | "photo_approved" | "photo_rejected";
  targetUserId?: string;
  metadata?: {
    choreName?: string;
    points?: number;
...
export const useNotifications = ({
  ...
  const triggerWorkflowNotification = useCallback(async ({
    choreId,
    type,
    targetUserId,
    metadata = {}
  ...
  const notifyChoreAssigned = useCallback((choreId: string, childUserId: string, choreName: string, points: number) => {
  ...
  const notifyChoreAccepted = useCallback((choreId: string, parentUserId: string, choreName: string) => {
    return triggerWorkflowNotification({
      choreId,
      type: "accepted",
  ...
  const notifyChoreDeclined = useCallback((choreId: string, parentUserId: string, choreName: string, reason: string) => {
    return triggerWorkflowNotification({
      choreId,
      type: "declined",
      targetUserId: parentUserId,
  ...
  const notifyChoreInProgress = useCallback((choreId: string, parentUserId: string, choreName: string) => {
    return triggerWorkflowNotification({
      choreId,
      type: "in_progress",
      targetUserId: parentUserId,
  ...
  const notifyChoreCompleted = useCallback((choreId: string, parentUserId: string, choreName: string, points: number) => {
    return triggerWorkflowNotification({
      choreId,
      type: "completed",
      targetUserId: parentUserId,
  ...
  const notifyPhotoSubmitted = useCallback((choreId: string, parentUserId: string, choreName: string, photoUrl: string) => {
    return triggerWorkflowNotification({
      choreId,
  ...
  const notifyPhotoApproved = useCallback((choreId: string, childUserId: string, choreName: string, points: number) => {
    return triggerWorkflowNotification({
      choreId,
      type: "photo_approved",
      targetUserId: childUserId,
  ...
  const notifyPhotoRejected = useCallback((choreId: string, childUserId: string, choreName: string, reason: string) => {
    return triggerWorkflowNotification({
      choreId,
      type: "photo_rejected",
      targetUserId: childUserId,
