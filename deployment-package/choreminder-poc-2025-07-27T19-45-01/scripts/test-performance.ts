...
class PerformanceTestSuite {
  ...
  private async runPerformanceTest(
    ...
    try {
      ...
      console.log(
        `${status === "PASS" ? "✅" : "❌"} ${testName}: ${duration}ms (threshold: ${thresholdMs}ms)`,
      );
      if (memoryDelta > 0) {
  ...
  private async setupPerformanceTestData(): Promise<void> {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const chore = new Chore({
        ...
        ), // Random creation date within 90 days
      });
      await chore.save();
      chores.push(chore);
    }
    ...
    console.log(
      `Created ${users.length} users and ${chores.length} chores for performance testing`,
    );
  }
  ...
  private async testDatabaseQueryPerformance(): Promise<void> {
    ...
    await this.runPerformanceTest(
      "Optimized Query with Performance Service",
      async () => {
  ...
  private async testBulkOperations(): Promise<void> {
    ...
    await this.runPerformanceTest(
      ...
      async () => {
        ...
        const results = await performanceService.batchInsert(
          Chore,
          testDocs,
          20,
        );
    ...
    await this.runPerformanceTest(
      ...
      async () => {
        ...
        if (memoryIncrease > 50 * 1024 * 1024) {
          throw new Error(
            `Potential memory leak: ${this.formatBytes(memoryIncrease)} increase`,
          );
        }
      },
      5000,
  ...
  private async testAnalyticsPerformance(): Promise<void> {
    ...
    await this.runPerformanceTest(
      ...
      async () => {
        for (const user of this.testData.users.slice(0, 3)) {
          await analyticsService.getTimeSeriesData(
            user._id.toString(),
            "month",
          );
        }
    ...
    await this.runPerformanceTest(
      "Family Analytics",
      async () => {
        await analyticsService.getFamilyAnalytics(
    ...
    await this.runPerformanceTest(
      ...
      async () => {
        for (const user of this.testData.users.slice(0, 3)) {
          await analyticsService.getCategoryInsights(
            user._id.toString(),
            "month",
          );
        }
  ...
  private generatePerformanceSummary(): {
    ...
    const totalMemoryDelta = this.results.reduce(
      (sum, r) => sum + Math.max(0, r.memoryUsage.delta),
      0,
    );
